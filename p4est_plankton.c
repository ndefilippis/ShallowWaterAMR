...
typedef struct step3_ctx
{
  double              center[P4EST_DIM];  /**< coordinates of the center of
                                               ...
                                               bump */
  double              center2[P4EST_DIM];  /**< coordinates of the center of
                                               the initial condition Gaussian
                                               bump */                                             
...
step3_initial_condition (double x[], double du[], double du2[], double du3[],
                         double retval[], step3_ctx_t * ctx)
{
  int                 i;
  double             *c = ctx->center;
  double             *c2 = ctx->center2;
  double              bump_width = ctx->bump_width;
  double              r2[3], d[P4EST_DIM];
  double              d2[P4EST_DIM];
  double              d3[P4EST_DIM];
  double              arg[3];
  ...
  arg[0] = -(1. / 2.) * r2[0] / bump_width / bump_width;
  retval[0] = exp (arg[0]);
  
  arg[1] = -(1. / 2.) * r2[1] / bump_width / bump_width;
  retval[1] = exp (arg[1]);

  arg[2] = -(1. / 2.) * r2[2] / bump_width / bump_width;
  retval[2] = 0.1 * exp (arg[2]);

  if (du) {
    for (i = 0; i < P4EST_DIM; i++) {
      du[i] = -(1. / bump_width / bump_width) * d[i] * retval[0];
      du2[i] = -(1. / bump_width / bump_width) * d2[i] * retval[1];
      du3[i] = -(1. / bump_width / bump_width) * d3[i] * retval[2];;
    }
  }
  //printf("%f\n", retval[0]);
}
                              ...
                              p4est_quadrant_t * q)
{
  ...
  double result[3];
  step3_initial_condition (midpoint, data->du, data->du2,
                                     data->du3, result, ctx); // does other derivatives
  data->u = result[0];
  data->u2 = result[1]; // initial zooplankton
  data->u3 = result[2]; // initial nutrients  
}
...
step3_error_sqr_estimate (p4est_quadrant_t * q)
{
  ...
  for (i = 0; i < P4EST_DIM; i++) {
    ...
    diff2 += du2[i] * du2[i] * (1. / 12.) * h * h * vol;
    diff2 += du3[i] * du3[i] * (1. / 12.) * h * h * vol;
  }
                                 ...
                                 p4est_quadrant_t * children[])
{
  ...
  double result[3];
  step3_initial_condition (parentmidpoint, parentdata.du,
                                          parentdata.du2,
                                          parentdata.du3, result, ctx);
  parentdata.u = result[0];
  parentdata.u2 = result[1];
  parentdata.u3 = result[2];
                            ...
                            p4est_quadrant_t * children[])
{
  ...
  for (i = 0; i < P4EST_CHILDREN; i++) {
    ...
    diff = (parentu - data->u2) * (parentu - data->u2);
    diff = (parentu - data->u3) * (parentu - data->u3);
    err2 += diff * vol;
  }
 ...
 * always 0.
 * Now user modified to a population growth scenario, so that this
 * may not be 0.
 *
...
step3_quad_divergence (p4est_iter_volume_info_t * info, void *user_data)
{
  ...
  double user_nu                    = 0.05; //user defined death rate
  double user_mu                    = 0.05;    // nutrient uptake rate
  double user_k                     = 0.8;    // nutrient max uptake loc.
  double user_tau                   = 4;    // nutrient restore timescale
  double user_star                  = 0.01;   // nutrient restore value

  data->du2dt  = 0.; //user note: does this need to be initialized to 0??
  data->du2dt += user_gamma*fabs((data->u2)*(data->u)); // z grow
  data->du2dt += -1.0*user_nu*(data->u2)*(data->u2); // z die
  
  data->dudt  = 0.;
  data->dudt += user_mu*(data->u3)*fabs(data->u)/((data->u3)+user_k);// p grow
  data->dudt += -1.0*user_gamma*fabs((data->u2)*(data->u)); // p die
  
  data->du3dt  = 0.;
  data->du3dt += (user_star - (data->u3))/user_tau; // n grow
  data->du3dt += -1.*user_mu*(data->u3)*fabs(data->u)/((data->u3)+user_k); // n die
...
step3_upwind_flux (p4est_iter_face_info_t * info, void *user_data)
{
  ...
  if(fabs(udata->du2dt) > 100 || fabs(udata->dudt) > 100 || fabs(udata->du3dt) > 100) {
    //printf("%f %f %f\n", udata->dudt, udata->du2dt , udata->du3dt);
    //exit(1);
  }
}
...
step3_timestep_update (p4est_iter_volume_info_t * info, void *user_data)
{
  ...
  data->u3 = SC_MAX(data->u3, 0);
  data->u2 = SC_MAX(data->u2, 0);
  data->u = SC_MAX(data->u, 0);
}
...
step3_minmod_estimate (p4est_iter_face_info_t * info, void *user_data)
{
  ...
  step3_data_t       *udata;
  p4est_quadrant_t   *quad;
  double              uavg[2], u2avg[2], u3avg[2];
  double              h[2];
  double              du_est, du_old, du2_est, du2_old, du3_est, du3_old;
  int                 which_dir;
  ...
  du2_est = (u2avg[1] - u2avg[0]) / ((h[0] + h[1]) / 2.);
  du3_est = (u3avg[1] - u3avg[0]) / ((h[0] + h[1]) / 2.);
  for (i = 0; i < 2; i++) {
    if (side[i]->is_hanging) {
      /* there are 2^(d-1) (P4EST_HALF) subfaces */
      for (j = 0; j < P4EST_HALF; j++) {
        ...
        if (!side[i]->is.hanging.is_ghost[j]) {
          ...
          du2_old = udata->du2[which_dir];
          du3_old = udata->du3[which_dir];
          if (du_old == du_old) { // once
          ...
          if (du2_old == du2_old) { //twice 
            ...
            if (du2_est * du2_old >= 0.) {
              if (fabs (du2_est) < fabs (du2_old)) {
                udata->du2[which_dir] = du2_est;
              }
            }
            else {
              udata->du2[which_dir] = 0.;
            }
          }
          else {
            udata->du2[which_dir] = du2_est;
          ...
          if (du3_old == du3_old) { //thrice
            ...
            if (du3_est * du3_old >= 0.) {
              if (fabs (du3_est) < fabs (du3_old)) {
                udata->du3[which_dir] = du3_est;
              }
            }
            else {
              udata->du3[which_dir] = 0.;
            }
          }
          else {
            udata->du3[which_dir] = du_est;
          }
        }
      }
    }
      ...
      if (!side[i]->is.full.is_ghost) {
        ...
        du2_old = udata->du2[which_dir]; //twice
        if (du2_old == du2_old) {
          /* there has already been an update */
          if (du2_est * du2_old >= 0.) {
            if (fabs (du2_est) < fabs (du2_old)) {
...
step3_compute_max (p4est_iter_volume_info_t * info, void *user_data)
{
  ...
  umax = SC_MAX (data->u2, umax);
  umax = SC_MAX (data->u3, umax);

  *((double *) user_data) = umax;
}

static void
step3_compute_dudt_max (p4est_iter_volume_info_t * info, void *user_data)
{
  p4est_quadrant_t   *q = info->quad;
  step3_data_t       *data = (step3_data_t *) q->p.user_data;
  double             *umax = (double *) user_data;

  umax[0] = SC_MAX (fabs(data->dudt), umax[0]);
  umax[1] = SC_MAX (fabs(data->du2dt), umax[1]);
  umax[2] = SC_MAX (fabs(data->du3dt), umax[2]);
}
...
//                 NULL);         /* there is no callback for the corners between quadrants */
  ...
  for (t = start_time, i = ctx->time_step; t < end_time; t += dt, i++) {
    ...
    double global_dumax[3];
    double dumax[3];
    dumax[0] = 0;
    dumax[1] = 0;
    dumax[2] = 0;
        /* initialize derivative estimates */
        p4est_iterate (p4est, NULL, (void *) &dumax,     /* pass in ghost data that we just exchanged */
                       step3_compute_dudt_max,       /* blank the previously calculated derivatives */
                       NULL,    /* there is no callback for the faces between quadrants */
#ifdef P4_TO_P8
                       NULL,    /* there is no callback for the edges between quadrants */
#endif
        ...
        mpiret =
          sc_MPI_Allreduce (&dumax, &global_dumax, 3, sc_MPI_DOUBLE, sc_MPI_MAX,
                            p4est->mpicomm);
        SC_CHECK_MPI (mpiret);
        P4EST_GLOBAL_PRODUCTIONF ("dudt_max %f %f %f\n", global_dumax[0], global_dumax[1], global_dumax[2]); 
